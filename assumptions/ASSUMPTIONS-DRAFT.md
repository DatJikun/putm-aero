...
**Baza:** wyłącznie ekstrakty w `sources/` + indeksy batch A/B (2026-09-01)  
...
Propozycja celu nadrzędnego (do decyzji):
...
| P3 | **Accel / proste** — niski drag przy wysokiej V lub DRS open | Jackson: DRS open −35% siły oporu vs closed; +15,7% potencjał accel @25 m/s; Vmax +18,2% vs fixed high-DF (**H**) | H (Jackson); **TBD** czy DRS w naszym regulaminie |

**Założenie robocze:** pakiet projektujemy pod **ważoną funkcję** (lap + energia), nie pod sam peak Cz @ δ=0. Staniszewski 2024 pokazuje, że Cx(δ) pełnego pakietu i FW&RW przecinają się ~10° — bez modelu toru nie da się wybrać zwycięzcy po samym Cx (**H**).

**TBD zespołu:** wagi eventów (Autocross / Endurance / Accel / Skidpad), masa bolidu, Crr, pojemność baterii, tor docelowy.

---

## 2. Zakres elementów (RW, FW, undertray, sidepods, DRS, fan — IN / OUT / TBD)
...
| **Wąsy / VG (Selig S1223)** | **IN (kandydat)** | iter111.4: Fz_all −565 N vs −561 N baseline, Cx 1,480, balans 57,3%; wąsy same dają Fz>0, ale chronią RW / wspierają UT | Nagłowski 2024 | H (Δ sił); M (rekomendacja „pozytywne dla rozwoju”) |
| **DRS (ruchome klapy RW)** | **TBD** | Jackson: closed CL_DF=1,15 / CD=1,21; open CL=0,26 / CD=0,79; −35% drag, +18,2% Vmax vs fixed high-DF. W pracach PUT **brak** DRS. Wymaga zgody regulaminu + aktuacji | Jackson 2018 | H (efekt na ich bolidzie); L (przeniesienie na nasz) |
...
Warunek odniesienia w pracach PUT: **V = 15 m/s**, steady RANS k-ε / Realizable k-ε (**H**). Jackson: inlet **26,8 m/s** — inne skalowanie (**H**).

| Metryka | Propozycja widełek | Źródło / komentarz | Pewność | Status |
|---------|-------------------|--------------------|---------|--------|
| Cz (pełny pakiet, δ=0) | **cel roboczy −3,5 … −4,2** (kierunek Nagłowski); nie mieszać z Cz≈−2,0 z innego modelu (Staniszewski 3D) | Nagłowski baseline −4,071 / iter111.4 −4,100 @15 m/s; Staniszewski 2023 pojazd ~−2,03 = **inny** pakiet/model | H (liczby źródeł); **L** jako target naszego auta bez własnego baseline | **TBD** — potrzebny baseline zespołu |
| Cx (pełny pakiet, δ=0) | **orientacyjnie 1,45–1,50** przy wysokim DF (Nagłowski); DRS open ~0,8 jeśli wdrożymy | Nagłowski Cx 1,453→1,480; Jackson DRS open CD=0,79 | H | **TBD** baseline |
...
| Balans aero przód | **55–60%** (utrzymać front-biased; wąsy cofają z ~60% → ~57%) | Nagłowski Tab. 5.2 | H | **TBD** target dynamiki / CG |
...
| Cornering / accel (jeśli DRS) | Jackson: +3,1% @R=13 m; +15,7% accel @25 m/s; −35% drag open | Jackson | H ich; L nasze | tylko jeśli DRS = IN |
...
*Koniec DRAFT. Kolejne kroki po review: uzupełnić TBD baseline zespołu → zamrozić IN/OUT → spiąć targety z wagami eventów.*
